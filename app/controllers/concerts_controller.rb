class ConcertsController < ApplicationController
  ...
  before_action :query_groups, only: %i[show edit new update]
  ...
  private
    ...
    def concert_params
      params.require(:concert).permit(:date, :assistants, :place, :duration, :group_id)
    end

    def query_groups
      @groups = Group.select(:id, :name).order(name: :asc)
    end
